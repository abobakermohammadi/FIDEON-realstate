(() => {
  ...
  function getSaved() {
    return safeJSON(localStorage.getItem(STORE.saved), []);
  }
  ...
  function initSaveButtons() {
    ...
    document.addEventListener("fideon:saved", syncSaveButtons);
    syncSaveButtons();
  }
    ...
    return `
      <article class="property-card ${large ? "large" : ""}" data-property-card data-type="${p.type}" data-intent="${p.intent}" data-visibility="${p.visibility}">
        <div class="property-image">
          <a href="${url}" aria-label="View ${escapeHTML(p.title)}">
            <img src="${escapeAttr(p.image || "/assets/property-palm.svg")}" alt="" loading="lazy">
          </a>
          <div class="property-badges">
            <span class="badge ${privateClass}">${escapeHTML(p.status || p.visibility || "Property")}</span>
            ${p.sample ? '<span class="badge">Sample</span>' : ""}
          </div>
        ...
        <div class="property-body">
          ...
          <div class="property-meta">
            <span class="property-price">${escapeHTML(p.priceLabel || "Price on request")}</span>
            ${p.beds ? `<span>${p.beds} beds</span>` : ""}
            ${p.baths ? `<span>${p.baths} baths</span>` : ""}
            ${p.area ? `<span>${escapeHTML(p.area)}</span>` : ""}
          </div>
          <div class="property-foot">
            <span class="sample-note">${p.sample ? "Development-only showcase inventory" : escapeHTML(p.type || "")}</span>
            <a class="btn-link" href="${url}">View</a>
          </div>
        </div>
      </article>`;
  }
  ...
  function initPropertyListing() {
    ...
    const search = $("[data-property-search]");
    ...
    const render = () => {
      ...
      if (filter !== "All") {
        items = items.filter(p => [p.type, p.intent, p.visibility].includes(filter));
      }
      const q = (search?.value || "").trim().toLowerCase();
      if (q) {
        items = items.filter(p => [p.title,p.location,p.type,p.intent,p.visibility].join(" ").toLowerCase().includes(q));
      }
      ...
      if (count) count.textContent = `${items.length} ${items.length === 1 ? "property" : "properties"}`;
      grid.innerHTML = items.length
        ? items.map(p => propertyCard(p)).join("")
        : `<div class="empty-state"><h3>No properties match this view.</h3><p>Clear the filter or ask FIDEON to source something privately.</p><a class="btn btn-dark" href="/find/">Start a private search</a></div>`;
      syncSaveButtons();
    };
    ...
    document.addEventListener("fideon:properties", render);
    render();
  }
  ...
  function initSavedPage() {
    ...
    const render = () => {
      ...
      grid.innerHTML = items.length
        ? items.map(p => propertyCard(p)).join("")
        : `<div class="empty-state"><h3>Your shortlist is quiet.</h3><p>Save properties to compare them here, without creating an account.</p><a class="btn btn-dark" href="/properties/">Explore properties</a></div>`;
      syncSaveButtons();
    };
    render();
    document.addEventListener("fideon:saved", render);
    document.addEventListener("fideon:properties", render);
  }
    ...
    $$('form[data-lead-form]').forEach(form => {
      form.addEventListener('submit', async e => {
        ...
        if (required.length || unchecked.length) {
          status.className = 'form-status show preview';
          status.textContent = 'Please complete the required fields before sending.';
          (required[0] || unchecked[0])?.focus();
          return;
        }
        const lead = {
          ...
          email: data.email || '',
          phone: data.phone || '',
          preferred: data.preferred || data.channel || 'Website',
          source: form.dataset.source || location.pathname,
          inquiryType: data.inquiryType || form.dataset.inquiryType || 'general',
          property: data.property || '',
          message: data.message || data.criteria || '',
          criteria: data
        };
        ...
        try {
          if (F.backend?.mode === 'production') {
            ...
            status.textContent = 'Request received. FIDEON will follow up through your preferred contact method.';
          } else {
            addLead({
              name: lead.name, email: lead.email, phone: lead.phone, channel: lead.preferred, source: lead.source,
              property: lead.property, stage: 'New', note: lead.message, payload: data
            });
            status.className = 'form-status show success';
            status.textContent = 'Saved in this browser. Localhost mode keeps this lead on this device.';
          }
          form.reset();
          toast('Request captured.');
        } catch (error) {
          status.className = 'form-status show preview';
          status.textContent = error.message || 'Could not send the request. Please try again.';
        } finally {
  ...
  function initDynamicPropertyDetail() {
    ...
    const render = () => {
      ...
      if (!p) {
        root.innerHTML = `<div class="empty-state"><h3>Property not available.</h3><p>The link may be private, expired, or no longer published.</p><a class="btn btn-dark" href="/properties/">Back to properties</a></div>`;
        return;
      }
      document.title = `${p.title} | FIDEON`;
      root.innerHTML = `<article class="dynamic-detail">
        <div class="dynamic-detail-media"><img src="${escapeAttr(p.image || '/assets/property-palm.svg')}" alt=""></div>
        <div class="dynamic-detail-copy">
          <div class="section-kicker">${escapeHTML(p.visibility || 'Property')}</div>
          <h1>${escapeHTML(p.title)}</h1>
          <p class="detail-location">${escapeHTML(p.location || [p.district,p.city,p.country].filter(Boolean).join(' · '))}</p>
          ...
          ${p.summary ? `<p class="detail-summary">${escapeHTML(p.summary)}</p>` : ''}
          ${p.description ? `<div class="long-copy"><p>${escapeHTML(p.description)}</p></div>` : ''}
          ${Array.isArray(p.highlights) && p.highlights.length ? `<div class="highlight-list">${p.highlights.map(h => `<div class="highlight">${escapeHTML(h)}</div>`).join('')}</div>` : ''}
          <div class="cta-actions"><a class="btn btn-gold" href="/contact/?property=${encodeURIComponent(p.slug || p.id)}">Request details</a><button class="btn btn-outline" data-save="${escapeAttr(p.id)}" type="button">Save property</button></div>
          <p class="sample-note">Sensitive location and availability details are disclosed only when verified and appropriate.</p>
        </div>
      </article>`;
      syncSaveButtons();
    };
  ...
  function initGallery() {
    const openers = $$("[data-gallery-open]");
    const modal = $("#gallery-modal");
    if (!modal || !openers.length) return;
    const img = $("img", modal);
    const close = $("[data-gallery-close]", modal);
    const open = source => {
      img.src = source;
    ...
    openers.forEach(btn => btn.addEventListener("click", () => open(btn.dataset.galleryOpen)));
    close?.addEventListener("click", hide);
  ...
  function initSearchRibbon() {
    $$("[data-search-submit]").forEach(btn => btn.addEventListener("click", () => {
      ...
      $$("select,input", panel || document).forEach(el => {
        if (el.value && el.name) params.set(el.name, el.value);
      });
